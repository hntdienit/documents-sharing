...
const ListLecturers = () => {
  ...
  useEffect(() => {
    newRequest.get(`/course?keyword=${keyword}&page=${page}&limit=${limit}`).then((res) => {
      if (res.data.error) {
  ...
  const deleteItem = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        newRequest.delete(`/course/${id}`).then((res) => {
          setList(
            list.filter((val) => {
              return val.id !== id;
            })
  ...
  return (
    <>
      <Card elevation={4}>
        <HeaderPage list title={"lớp"} to={"/admin/course/create"} setKeyword={setKeyword} />
                ...
                <TableBody>
                  {list.map((item, index) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell align="center">{item?.Ma_lop}</TableCell>
                      <TableCell align="center">{item?.Ten_lop_hoc}</TableCell>
                      <TableCell align="center">
