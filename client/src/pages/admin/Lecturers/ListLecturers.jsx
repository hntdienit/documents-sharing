...
const ListLecturers = () => {
  ...
  useEffect(() => {
    newRequest.get(`/admin/lecturers?keyword=${keyword}&page=${page}&limit=${limit}`).then((res) => {
      if (res.data.error) {
  ...
  const deleteItem = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        newRequest.delete(`/admin/user/${id}`).then((res) => {
          setList(
            list.filter((val) => {
              return val.id !== id;
            })
  ...
  return (
    <>
      <Card elevation={4}>
        <HeaderPage list title={"giảng viên"} to={"/admin/lecturers/create"} setKeyword={setKeyword} />
                ...
                <TableBody>
                  {list.map((item, index) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell align="center">{item?.Ho_ten}</TableCell>
                      <TableCell align="center">{item?.Dia_chi}</TableCell>
                      <TableCell align="center">{item?.CCCD}</TableCell>
                      <TableCell align="center">{item?.Gioi_tinh}</TableCell>
                      <TableCell align="center">{item?.So_dien_thoai}</TableCell>
                      <TableCell align="center">
                        <Link to={`/admin/lecturers/edit/${item.id}`}>
