...
const ListMajor = () => {
  ...
  useEffect(() => {
    newRequest.get(`/major?keyword=${keyword}&page=${page}&limit=${limit}`).then((res) => {
      if (res.data.error) {
  ...
  const deleteItem = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        newRequest.delete(`/major/${id}`).then((res) => {
          setList(
            list.filter((val) => {
              return val.id !== id;
            })
  ...
  return (
    <>
      <Card elevation={4}>
        <HeaderPage list title={"ngành học"} to={"/admin/major/create"} setKeyword={setKeyword} />
                ...
                <TableBody>
                  {list.map((item, index) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell align="center">{item?.Ma_nganh_hoc}</TableCell>
